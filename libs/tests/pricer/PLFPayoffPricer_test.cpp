...
TEST_F(PLFPayoffPricerTest, MultiplyPayment) {
    constexpr double K = 100.0;
    ...
    {
        const double pricerPrice = hestonPricer(payoff, market, hestonParams);
        const double expected = multiplier * hestonCallFormula(forward1, K, T1, dF1, hestonParams);

        EXPECT_NEAR(pricerPrice, expected, 1e-10);
    }
}

TEST_F(PLFPayoffPricerTest, CombinedAndMultiPaymentNested) {
    constexpr double K = 100.0;
