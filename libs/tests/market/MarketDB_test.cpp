...
TEST_F(MarketDBTest, BSVolSlice_ExactDate_VolIsPositive) {
    ...
    for (const auto K : {80.0, 90.0, 100.0, 110.0, 120.0}) {
...
TEST_F(MarketDBTest, BSVolSlice_Caching) {
    const auto& s1 = mkt.getBSVolSlice("SPX", makeDate(2026, 5, 24));
    const auto& s2 = mkt.getBSVolSlice("SPX", makeDate(2026, 5, 24));
    EXPECT_EQ(&s1, &s2);
}
...
TEST_F(MarketDBTest, BSVolSlice_TemporalInterpolation) {
    const auto& nearSlice = mkt.getBSVolSlice("SPX", makeDate(2026, 5, 24));
    const auto& farSlice = mkt.getBSVolSlice("SPX", makeDate(2026, 7, 24));
    const auto& midSlice = mkt.getBSVolSlice("SPX", makeDate(2026, 6, 24));

    const auto T1 = nearSlice.time();
    const auto T2 = farSlice.time();
    const auto T = midSlice.time();
    const auto alpha = (T - T1) / (T2 - T1);

    for (const auto K : {85.0, 95.0, 100.0, 105.0, 115.0}) {
        const auto w1 = nearSlice.vol(K) * nearSlice.vol(K) * T1;
        const auto w2 = farSlice.vol(K)  * farSlice.vol(K)  * T2;
        const auto wExpected = w1 + alpha * (w2 - w1);
...
TEST_F(MarketDBTest, BSVolSlice_ExtrapolationBeyondLastDate) {
    const auto& lastSlice = mkt.getBSVolSlice("SPX", makeDate(2027, 4, 24));
    const auto& beyondSlice = mkt.getBSVolSlice("SPX", makeDate(2028, 1, 1));
    for (const auto K : {80.0, 100.0, 120.0}) {
...
TEST_F(MarketDBTest, BSVolSlice_ExtrapolationBeforeFirstDate) {
    const auto& firstSlice = mkt.getBSVolSlice("SPX", makeDate(2026, 5, 24));
    const auto& beforeSlice = mkt.getBSVolSlice("SPX", makeDate(2026, 5, 1));
    for (const auto K : {80.0, 100.0, 120.0}) {
